  ...
  return (
    <div className="grid grid-cols-4 gap-6 mt-8">
      {stats.map((item, index) => (
        <div
          key={index}
          className="relative bg-white border border-[#E0E0E0] rounded-[20px] h-[215px] overflow-hidden px-7 pt-6"
        >
          {/* HEADER */}
          <div className="flex justify-between items-start">
            {/* TEXT */}
            <div className="text-right flex flex-col w-full ">
              <h3 className="text-[20px] font-medium text-black mr-1 leading-none">
                {item.title}
              </h3>
              ...
              <div className="flex items-center gap-2 mt-5 mb-15">
                <img
                  src="/Icons/Up.png"
                  alt="arrow"
                  className="w-[23px] h-[21px] object-contain"
                />

                <span className="text-[20px] text-[#216474] font-medium">
                  +15%
                </span>

                <span className="text-[16px] text-[#717171] font-medium">
                  عن الشهر الماضي
                </span>
              </div>
            </div>
