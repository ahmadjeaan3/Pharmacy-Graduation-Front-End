...
export function ManualInventoryDialog({ pending, onClose, onSave }) {
  ...
  const language = String(i18n.resolvedLanguage || i18n.language || "ar").split(
    "-",
  )[0];
  ...
  return (
    ...
    >
      <div
        ...
        className="max-h-[94vh] w-full max-w-5xl overflow-auto rounded-[1.75rem] bg-white shadow-2xl"
      >
        <header className="sticky top-0 z-10 flex items-start justify-between gap-4 border-b border-[#174b57]/8 bg-white/95 p-5 backdrop-blur sm:p-6">
          <div className="flex items-center gap-3">
            ...
            <div>
              <h2 className="text-xl font-black text-[#17363e]">
                {t("إضافة دواء يدويًا")}
              </h2>
              <p className="mt-1 text-xs leading-5 text-[#71858a]">
                {t("استخدمها عندما لا تجد الدواء في الدليل المركزي.")}
              </p>
            </div>
          </div>
          <button
        ...
        >
          <FormSection
            ...
            title={t("هوية الدواء")}
            description={t(
              "أدخل الاسم الأساسي ثم أضف العربية لتسهيل البحث للمستخدم.",
            )}
          >
            <Field
              label={t("الاسم التجاري بالإنجليزية")}
              required
              value={form.name}
            ...
            />
            <Field
              label={t("الاسم التجاري بالعربية")}
              value={form.arabicName}
              onChange={change("arabicName")}
              maxLength={500}
            />
            <Field
              label={t("الاسم العلمي بالإنجليزية")}
            ...
            />
            <Field
              label={t("الاسم العلمي بالعربية")}
              value={form.arabicScientificName}
              onChange={change("arabicScientificName")}
              maxLength={2000}
            />
            <Field
              label={t("الباركود")}
              value={form.barcode}
              onChange={change("barcode")}
              maxLength={64}
              pattern="[0-9A-Za-z-]+"
              dir="ltr"
            />
            <Field
              label={t("الشركة المصنعة")}
          ...
          <FormSection
            icon={PackagePlus}
            title={t("الشكل والتعبئة")}
            description={t(
              "بيانات تساعد على تمييز العبوة الصحيحة داخل المخزون.",
            )}
          >
            <Field
              label={t("الشكل الدوائي")}
              value={form.dosageForm}
              onChange={change("dosageForm")}
              maxLength={100}
              placeholder={t("أقراص، شراب، حقن...")}
            />
            <Field
              label={t("حجم العبوة")}
            ...
            />
            <Field
              label={t("السعة أو التركيز")}
              value={form.capacity}
              onChange={change("capacity")}
              maxLength={100}
            />
            <Toggle
              label={t("يتطلب وصفة طبية")}
          ...
          <FormSection
            icon={PackagePlus}
            title={t("بيانات المخزون")}
            description={t("حدد الكمية وسعر هذا الدواء داخل صيدليتك.")}
          >
            <Field
              type="number"
              min="0"
            ...
            <Field
              ...
              min="0"
              max="100000"
              label={t("حد تنبيه انخفاض المخزون")}
              required
            ...
            />
            <Toggle
              label={t("متاح للطلبات")}
              checked={form.isAvailable}
              onChange={change("isAvailable")}
            />
            <Toggle
              label={t("إظهار السعر للمستخدمين")}
          ...
          <div className="grid gap-4 md:grid-cols-2">
            <label>
              <span className="form-label">{t("التركيب")}</span>
              <textarea
            ...
            <label>
              <span className="form-label">{t("الوصف")}</span>
              <textarea
                className="form-input min-h-28 resize-y"
                maxLength={1000}
          ...
          <div className="flex flex-col-reverse gap-2 border-t border-[#174b57]/8 pt-5 sm:flex-row sm:justify-end">
            ...
            >
              <PackagePlus size={17} />
              {pending ? t("جاري الحفظ...") : t("إضافة إلى المخزون")}
            </button>
          </div>
        </form>
      </div>
  ...
  return (
    <section className="rounded-[1.35rem] border border-[#e1ebed] bg-[#fafcfc] p-4 sm:p-5">
      <div className="mb-4 flex items-center gap-3">
        ...
        <div>
          <h3 className="text-sm font-black text-[#29464d]">{title}</h3>
          <p className="mt-0.5 text-xs text-[#829499]">{description}</p>
        </div>
      </div>
