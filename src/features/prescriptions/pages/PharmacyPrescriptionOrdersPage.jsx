...
export function PharmacyPrescriptionOrdersPage() {
  ...
  const update = useMutation({
    mutationFn: ({ id, ...payload }) => updatePrescriptionStatus(id, payload),
    onSuccess: () =>
      client.invalidateQueries({ queryKey: prescriptionKeys.pharmacy }),
  });
  return (
    <div className="space-y-6">
      <header>
        <span className="eyebrow">الوصفات المحجوزة</span>
        <h1 className="mt-2 text-3xl font-black text-[#173f49]">
          تجهيز واستلام الوصفات
        </h1>
          ...
          <article key={order.id} className="surface p-5">
            <div className="flex flex-wrap justify-between gap-3">
              <div>
                ...
                <p className="text-xs text-[#829499]">
                  {order.items.length} أدوية — مطابقة {order.matchPercentage}%
                </p>
              </div>
            ...
            <div className="mt-4 grid gap-2 sm:grid-cols-2">
              {order.items.map((item) => (
                <div
                  key={item.id}
                  className="rounded-xl bg-[#f7faf9] p-3 text-sm"
                >
                  {item.matchedMedicineName || item.extractedName} —{" "}
            ...
            {order.status === "Reserved" && (
              <button
                ...
                onClick={() =>
                  update.mutate({ id: order.id, status: "ReadyForPickup" })
                }
              >
                <Clock3 size={17} /> تم تجهيز الوصفة
              </button>
            )}
                ...
                <button
                  ...
                  onClick={() =>
                    update.mutate({
                      id: order.id,
                      status: "Collected",
                      pickupCode: codes[order.id],
                    })
                  }
                >
                  <CheckCircle2 size={17} /> تأكيد الاستلام
                </button>
              </div>
            )}
          </article>
