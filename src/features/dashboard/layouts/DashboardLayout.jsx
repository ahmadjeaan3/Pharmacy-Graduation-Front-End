...
export function DashboardLayout() {
  ...
  return (
    <div className="min-h-screen bg-[#f4f7f6] text-[#17363e]">
      <button
        className="fixed right-4 top-4 z-50 grid size-11 place-items-center rounded-xl bg-white shadow-lg lg:hidden"
      ...
      <aside
        className={`fixed inset-y-0 right-0 z-40 flex w-[286px] flex-col overflow-hidden border-l border-white/10 bg-[#123f49] px-5 py-6 text-white shadow-2xl transition-transform lg:translate-x-0 ${open ? "translate-x-0" : "translate-x-full"}`}
      >
        <Brand light />
        <div className="mt-9 rounded-2xl border border-white/10 bg-white/[.065] p-4">
          <div className="flex items-center gap-3">
            <span className="grid size-11 place-items-center rounded-xl bg-[#f5cb72] font-black text-[#173d46]">
              {user.fullName?.trim()?.[0] || "ح"}
            </span>
        ...
        <nav
          className="dashboard-nav-scroll min-h-0 flex-1 space-y-1 overflow-y-auto pb-3 pl-2"
            ...
            >
              ...
              {to === "/app/notifications" && unreadCount > 0 ? (
                <span className="grid min-w-6 place-items-center rounded-full bg-rose-500 px-1.5 text-[10px] font-black leading-5 text-white">
                  {unreadCount > 99
                    ? "+99"
                    : unreadCount.toLocaleString("ar-SY")}
                </span>
              ) : (
                <ChevronDown
                  size={14}
