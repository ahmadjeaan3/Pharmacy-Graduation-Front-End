export const inventoryMessages = {
  en: {
    "مساحة الصيدلية": "Pharmacy workspace",
    "مخزون الأدوية": "Medicine inventory",
    "راجع التوفر والأسعار وتواريخ الصلاحية من لوحة واحدة مرتبة.":
      "Review availability, prices, and expiry dates from one organized workspace.",
    "إظهار العربية": "Show Arabic",
    "العربية ظاهرة": "Arabic visible",
    "تحديث": "Refresh",
    "إضافة من دليل الأدوية": "Add from medicine catalog",
    "إضافة دواء يدويًا": "Add medicine manually",
    "استيراد ملف Excel": "Import Excel file",
    "إجمالي الأصناف": "Total items",
    "متوفر": "In stock",
    "مخزون منخفض": "Low stock",
    "نافد": "Out of stock",
    "منتهي الصلاحية": "Expired",
    "هذا الصنف منتهي الصلاحية وغير متاح للطلبات حتى تحديث بياناته.":
      "This item has expired and cannot be ordered until its details are updated.",
    "{{out}} نافد و{{expired}} منتهي الصلاحية":
      "{{out}} out of stock and {{expired}} expired",
    "انتهت صلاحية هذا الصنف ويجب عزله عن الطلبات":
      "This item has expired and must be excluded from orders",
    "ابحث بالاسم الإنجليزي أو العربي أو العلمي أو الباركود":
      "Search by English, Arabic, or scientific name, or by barcode",
    "مسح الفلاتر": "Clear filters",
    "الكل": "All",
    "إظهار الأصناف المتوفرة فقط": "Show in-stock items only",
    "الاسم الإنجليزي أساسي، ويمكن إظهار الاسم العربي عند الحاجة.":
      "English is the primary name; you can show the Arabic name when needed.",
    "بدائل دوائية مقترحة": "Suggested medicine alternatives",
    "لم يتم التعرف على الدواء في الدليل المعتمد. جرّب الاسم التجاري أو العربي الكامل.":
      "The medicine was not recognized in the approved catalog. Try its full brand or Arabic name.",
    "بيانات الدواء غير كافية لتقديم بدائل آمنة ومتطابقة.":
      "The medicine data is insufficient to provide safe, matching alternatives.",
    "المادة الفعالة مطابقة": "Same active ingredient",
    "العيار مطابق": "Same strength",
    "الشكل الدوائي مطابق": "Same dosage form",
    "تحليل المخزون الذكي": "Smart inventory analysis",
    "الكمية الحالية": "Current quantity",
    "المباع إجمالًا": "Total sold",
    "متوسط الاستهلاك اليومي": "Average daily consumption",
    "مبيعات آخر 7 أيام": "Sales in the last 7 days",
    "مبيعات آخر 30 يومًا": "Sales in the last 30 days",
    "جاري التحليل...": "Analyzing...",
    "توقع موعد نفاد الدواء": "Predict stockout date",
    "درجة الخطورة": "Risk level",
    "المدة المتوقعة": "Estimated duration",
    "كمية إعادة الطلب": "Recommended reorder quantity",
    "النتيجة تقديرية وتعتمد على دقة أرقام المبيعات المدخلة، ويجب مراجعتها قبل اعتماد طلب التوريد.":
      "This estimate depends on the entered sales figures and should be reviewed before placing a supply order.",
    "محرك البدائل الدوائية": "Medicine alternatives engine",
    "اقتراح البدائل": "Suggest alternatives",
    "عرض البدائل": "View alternatives",
    "جاري تحليل المادة والتركيز والشكل الدوائي...":
      "Analyzing the ingredient, strength, and dosage form...",
    "لم يتم العثور على بدائل موثوقة لهذا الدواء.":
      "No reliable alternatives were found for this medicine.",
    تطابق: "Match",
    "هذه اقتراحات مساعدة فقط. لا يتم استبدال الدواء أو تغيير الجرعة إلا بعد مراجعة صيدلي أو طبيب مختص.":
      "These suggestions are for assistance only. Do not replace a medicine or change its dose without consulting a pharmacist or physician.",
    "إضافة دواء بالباركود": "Add medicine by barcode",
    "إضافة بالباركود": "Add by barcode",
    "امسح الباركود بالقارئ أو أدخله يدويًا، ثم اضغط بحث.":
      "Scan the barcode or enter it manually, then select Search.",
    "رقم الباركود": "Barcode number",
    "مثال: 8691234567890": "Example: 8691234567890",
    "جاري البحث...": "Searching...",
    "البحث وإضافة الدواء": "Search and add medicine",
    "لم يتم العثور على دواء مسجل بهذا الباركود.":
      "No registered medicine was found with this barcode.",
    "استيراد مخزون من Excel": "Import inventory from Excel",
    "استيراد Excel": "Import Excel",
    "حتى 100 دواء في كل ملف": "Up to 100 medicines per file",
    "الأعمدة المطلوبة": "Required columns",
    "اختيار ملف Excel": "Choose Excel file",
    "جاري قراءة الملف...": "Reading file...",
    "جاهز للاستيراد": "Ready to import",
    "جاري الاستيراد...": "Importing...",
    "استيراد {{count}} دواء": "Import {{count}} medicines",
    "الصف {{row}}: الباركود أو الكمية أو السعر غير صالح.":
      "Row {{row}}: the barcode, quantity, or price is invalid.",
    "الصف {{row}}: لا يوجد دواء مسجل بالباركود {{barcode}}.":
      "Row {{row}}: no registered medicine has barcode {{barcode}}.",
    "تم تجاهل الباركودات المكررة والإبقاء على آخر صف لكل دواء.":
      "Duplicate barcodes were ignored and the last row for each medicine was kept.",
    "تعذر قراءة الملف. استخدم ملف Excel بصيغة .xlsx وبعناوين الأعمدة المطلوبة.":
      "The file could not be read. Use an .xlsx file with the required column headers.",
    "جاري البحث في دليل الأدوية...": "Searching the medicine catalog...",
    "لم يتم العثور على دواء مطابق في الدليل المعتمد.":
      "No matching medicine was found in the approved catalog.",
    حرج: "Critical",
    مرتفع: "High",
    متوسط: "Medium",
    منخفض: "Low",
    يوم: "day",
    دواء: "medicine",
  },
  tr: {
    "مساحة الصيدلية": "Eczane çalışma alanı",
    "مخزون الأدوية": "İlaç envanteri",
    "راجع التوفر والأسعار وتواريخ الصلاحية من لوحة واحدة مرتبة.":
      "Stok, fiyat ve son kullanma tarihlerini düzenli bir ekrandan yönetin.",
    "إظهار العربية": "Arapçayı göster",
    "العربية ظاهرة": "Arapça görünür",
    "تحديث": "Yenile",
    "إضافة من دليل الأدوية": "İlaç kataloğundan ekle",
    "إضافة دواء يدويًا": "İlacı elle ekle",
    "استيراد ملف Excel": "Excel dosyası içe aktar",
    "إجمالي الأصناف": "Toplam ürün",
    "متوفر": "Stokta",
    "مخزون منخفض": "Düşük stok",
    "نافد": "Stokta yok",
    "منتهي الصلاحية": "Son kullanma tarihi geçmiş",
    "هذا الصنف منتهي الصلاحية وغير متاح للطلبات حتى تحديث بياناته.":
      "Bu ürünün son kullanma tarihi geçmiştir; bilgileri güncellenene kadar sipariş edilemez.",
    "{{out}} نافد و{{expired}} منتهي الصلاحية":
      "{{out}} stokta yok ve {{expired}} son kullanma tarihi geçmiş",
    "انتهت صلاحية هذا الصنف ويجب عزله عن الطلبات":
      "Bu ürünün son kullanma tarihi geçmiştir ve siparişlerden çıkarılmalıdır",
    "ابحث بالاسم الإنجليزي أو العربي أو العلمي أو الباركود":
      "İngilizce, Arapça veya bilimsel adla ya da barkodla ara",
    "مسح الفلاتر": "Filtreleri temizle",
    "الكل": "Tümü",
    "إظهار الأصناف المتوفرة فقط": "Yalnızca stoktakileri göster",
    "الاسم الإنجليزي أساسي، ويمكن إظهار الاسم العربي عند الحاجة.":
      "İngilizce ad birincildir; gerektiğinde Arapça ad gösterilebilir.",
    "بدائل دوائية مقترحة": "Önerilen ilaç alternatifleri",
    "لم يتم التعرف على الدواء في الدليل المعتمد. جرّب الاسم التجاري أو العربي الكامل.":
      "İlaç onaylı katalogda tanınmadı. Tam ticari veya Arapça adını deneyin.",
    "بيانات الدواء غير كافية لتقديم بدائل آمنة ومتطابقة.":
      "Güvenli ve eşleşen alternatifler sunmak için ilaç verileri yetersiz.",
    "المادة الفعالة مطابقة": "Aynı etken madde",
    "العيار مطابق": "Aynı doz",
    "الشكل الدوائي مطابق": "Aynı farmasötik form",
    "تحليل المخزون الذكي": "Akıllı stok analizi",
    "الكمية الحالية": "Mevcut miktar",
    "المباع إجمالًا": "Toplam satılan",
    "متوسط الاستهلاك اليومي": "Ortalama günlük tüketim",
    "مبيعات آخر 7 أيام": "Son 7 gün satışları",
    "مبيعات آخر 30 يومًا": "Son 30 gün satışları",
    "جاري التحليل...": "Analiz ediliyor...",
    "توقع موعد نفاد الدواء": "Stok tükenme tarihini tahmin et",
    "درجة الخطورة": "Risk seviyesi",
    "المدة المتوقعة": "Tahmini süre",
    "كمية إعادة الطلب": "Önerilen yeniden sipariş miktarı",
    "النتيجة تقديرية وتعتمد على دقة أرقام المبيعات المدخلة، ويجب مراجعتها قبل اعتماد طلب التوريد.":
      "Bu tahmin girilen satış verilerine bağlıdır ve tedarik siparişi verilmeden önce gözden geçirilmelidir.",
    "محرك البدائل الدوائية": "İlaç alternatifleri motoru",
    "اقتراح البدائل": "Alternatif öner",
    "عرض البدائل": "Alternatifleri göster",
    "جاري تحليل المادة والتركيز والشكل الدوائي...":
      "Etken madde, doz ve ilaç formu analiz ediliyor...",
    "لم يتم العثور على بدائل موثوقة لهذا الدواء.":
      "Bu ilaç için güvenilir bir alternatif bulunamadı.",
    تطابق: "Eşleşme",
    "هذه اقتراحات مساعدة فقط. لا يتم استبدال الدواء أو تغيير الجرعة إلا بعد مراجعة صيدلي أو طبيب مختص.":
      "Bu öneriler yalnızca yardımcı amaçlıdır. Eczacıya veya doktora danışmadan ilaç ya da doz değiştirilmemelidir.",
    "إضافة دواء بالباركود": "Barkodla ilaç ekle",
    "إضافة بالباركود": "Barkodla ekle",
    "امسح الباركود بالقارئ أو أدخله يدويًا، ثم اضغط بحث.":
      "Barkodu okutun veya elle girin, ardından Ara'yı seçin.",
    "رقم الباركود": "Barkod numarası",
    "مثال: 8691234567890": "Örnek: 8691234567890",
    "جاري البحث...": "Aranıyor...",
    "البحث وإضافة الدواء": "İlacı ara ve ekle",
    "لم يتم العثور على دواء مسجل بهذا الباركود.":
      "Bu barkoda kayıtlı bir ilaç bulunamadı.",
    "استيراد مخزون من Excel": "Excel'den stok içe aktar",
    "استيراد Excel": "Excel içe aktar",
    "حتى 100 دواء في كل ملف": "Dosya başına en fazla 100 ilaç",
    "الأعمدة المطلوبة": "Gerekli sütunlar",
    "اختيار ملف Excel": "Excel dosyası seç",
    "جاري قراءة الملف...": "Dosya okunuyor...",
    "جاهز للاستيراد": "İçe aktarmaya hazır",
    "جاري الاستيراد...": "İçe aktarılıyor...",
    "استيراد {{count}} دواء": "{{count}} ilacı içe aktar",
    "الصف {{row}}: الباركود أو الكمية أو السعر غير صالح.":
      "{{row}}. satır: barkod, miktar veya fiyat geçersiz.",
    "الصف {{row}}: لا يوجد دواء مسجل بالباركود {{barcode}}.":
      "{{row}}. satır: {{barcode}} barkoduna kayıtlı ilaç yok.",
    "تم تجاهل الباركودات المكررة والإبقاء على آخر صف لكل دواء.":
      "Yinelenen barkodlar yok sayıldı ve her ilaç için son satır korundu.",
    "تعذر قراءة الملف. استخدم ملف Excel بصيغة .xlsx وبعناوين الأعمدة المطلوبة.":
      "Dosya okunamadı. Gerekli sütun başlıklarını içeren bir .xlsx dosyası kullanın.",
    "جاري البحث في دليل الأدوية...": "İlaç kataloğunda aranıyor...",
    "لم يتم العثور على دواء مطابق في الدليل المعتمد.":
      "Onaylı katalogda eşleşen bir ilaç bulunamadı.",
    حرج: "Kritik",
    مرتفع: "Yüksek",
    متوسط: "Orta",
    منخفض: "Düşük",
    يوم: "gün",
    دواء: "ilaç",
  },
};
